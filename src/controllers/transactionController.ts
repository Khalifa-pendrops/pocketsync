  ...
  try {
    ...
    if (accountId && typeof accountId === "string") {
      ...
      if (!account) {
        res
          .status(403)
          .json({ error: "Forbidden — account not found or not yours" });
        return;
      }
    ...
    const safeCategory =
      ...
      (TRANSACTION_CATEGORIES as readonly string[]).includes(category)
        ...
        : undefined;
    const safeType =
      typeof type === "string" && (type === "credit" || type === "debit")
        ? type
        : undefined;
...
export const transferMoney = async (
  ...
  try {
    ...
    if (!fromAccountId || !toAccountId) {
      res
        .status(400)
        .json({ error: "fromAccountId and toAccountId are required" });
      return;
    }
    ...
    if (amountNgn === null) {
      res
        .status(400)
        .json({ error: "amount must be a number of at least ₦1.00" });
      return;
    }
    ...
    const debitResult = await debitAccountAtomic(
      userId,
      fromAccountId,
      amountKobo,
    );
    ...
    const creditedAccount = await creditAccountAtomic(
      userId,
      toAccountId,
      amountKobo,
    );
    if (!creditedAccount) {
      ...
      res
        .status(403)
        .json({
          error: "Forbidden — one or both accounts not found or not yours",
        });
      return;
    }
...
export const interbankTransfer = async (
  ...
  try {
    ...
    if (!recipientBankName) {
      res.status(400).json({
        error:
          "recipientBank is required and must be a valid bank name (2–100 characters)",
      });
      return;
    }
    ...
    if (
      ...
      res
        .status(400)
        .json({
          error: "recipientAccountNumber must be a 10-digit account number",
        });
      return;
    }
...
export const updateCategory = async (
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Category updated", category: transaction.category });
  } catch (err) {
